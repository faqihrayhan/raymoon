...
export function Footer() {
  ...
  return (
    <footer className="py-16 px-6 max-w-7xl mx-auto border-t border-white/[0.08] text-base font-mono text-[#9ca3af]">
        ...
        <div className="md:col-span-6 space-y-4">
          <div className="flex items-center text-white">
            <span className="font-bold text-xl sm:text-2xl tracking-tight">
              Raymoon<span className="inline-block w-2.5 h-2.5 rounded-full bg-orange-500 ml-0.5" />
            </span>
          </div>
          ...
          <ul className="space-y-2.5 text-sm">
            {socials.map((soc) => (
              <li key={soc.label}>
                ...
                >
                  <span>{soc.label}</span>
                  <ArrowUpRight className="w-3.5 h-3.5 text-[#555861] group-hover:text-white transition-colors" />
                </a>
              </li>
            ))}
          </ul>
